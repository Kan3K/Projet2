...
const estDansUnFormatValideb = (strDonnee, strFormat) => {
    let str = strFormat.replaceAll("#", "\[0-9]{1}");
    str = str.replaceAll("@", "\[a-zA-Z]{1}");
    const regex = new RegExp(str);
    if(regex.test(strDonnee)){return true;}
    return false
    }
...
const estBudget37 = (strDonnee) => {
    var objRegExp  = /^[0-9]*$/;
    //3 à 7 chiffres et >=500
    if (strDonnee.length>=3  && strDonnee.length<=7 && strDonnee>=500) {return objRegExp.test(strDonnee);}
    //return false;   
    return false;
}
const estVillchk = (strDonnee) => {
    var objRegExp  = /^[a-zA-Z]{1}[a-zA-Z]*(?=.*[-])[a-zA-Z]*|^[a-zA-Z]*$/;
    if (strDonnee.length ==0 || strDonnee.substr(strDonnee.length-1)=="-")   {return false}
    return objRegExp.test(strDonnee);
}
